...
ret = models.Book_list.objects.all().aggregate(Avg('book_price'))
print(ret)
...
#  查询每一本书的作者的数量
ret = models.Book_list.objects.all().annotate(author_num = Count('author'))
for book in ret :
    print(book.book_name,book.author_num)

#   查询作者数量大于一的书籍
ret = models.Book_list.objects.all().annotate(author_num = Count('author')).filter(author_num__gt=1)
for book in ret :
    print(book.book_name,book.author_num)

#   查询各个作者出的书总价格
ret = models.Author.objects.all().annotate(price_num = Sum('book__book_price')).values_list('name','price_num')
print(ret)